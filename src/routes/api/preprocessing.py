...
@preprocessing_bp.route('/imputation')
def imputation():
...
@preprocessing_bp.route('/threshold', methods=['POST'])
def threshold():
    with data_manager.processing_lock():
        ...
        session['current_shape'] = df_thresholded.shape
...
@preprocessing_bp.route('/apply_imputation', methods=['POST'])
def apply_imputation():
    with data_manager.processing_lock():
        ...
        try:
            ...
            session['current_shape'] = new_shape
...
@preprocessing_bp.route('/replace_zeros', methods=['POST'])
def replace_zeros():
    with data_manager.processing_lock():
        history_paths = session.get('df_history_paths', [])
        if not history_paths:
...
@preprocessing_bp.route('/normalization')
def normalization():
    history_paths = session.get('df_history_paths', [])
    if not history_paths:
        flash('Please define sample data first','warning')
...
@preprocessing_bp.route('/apply_normalization', methods=['POST'])
...
@preprocessing_bp.route('/transformation')
def transformation():
    history_paths = session.get('df_history_paths', [])
    if not history_paths:
        flash('Please define sample data first','warning')
...
@preprocessing_bp.route('/apply_transformation', methods=['POST'])
...
@preprocessing_bp.route('/scaling')
def scaling():
    history_paths = session.get('df_history_paths', [])
    if not history_paths:
        flash('Please define sample data first','warning')
...
@preprocessing_bp.route('/apply_scaling', methods=['POST'])
def apply_scaling():
    with data_manager.processing_lock():
        history_paths = session.get('df_history_paths', [])
        if not history_paths:
...
@preprocessing_bp.route('/reset_sample_step', methods=['POST'])
def reset_sample_step():
    with data_manager.processing_lock():
        ...
        df_current = data_manager.load_dataframe(history_paths[-1])
        session['current_shape'] = df_current.shape
        response_data = {
            'success': True,
